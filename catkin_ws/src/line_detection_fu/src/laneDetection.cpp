    ...
    : nh_(nh), priv_nh_("~")
{
    ...
    // maxYRoi=cam_h-proj_y_start;
    maxXRoi=cam_w;
    priv_nh_.param<int>(node_name+"/roi_top_w", roi_top_w, 640);
    priv_nh_.param<int>(node_name+"/roi_bottom_w", roi_bottom_w, 640);
    priv_nh_.param<int>(node_name+"/minYPolyRoi", minYPolyRoi, 120);
    priv_nh_.param<int>(node_name+"/maxYRoi", maxYRoi, 480);
    proj_image_h=maxYRoi;

    priv_nh_.param<int>(node_name+"/defaultXLeft", defaultXLeft, -50);
    priv_nh_.param<int>(node_name+"/defaultXCenter", defaultXCenter, 200);
    priv_nh_.param<int>(node_name+"/defaultXRight", defaultXRight, 450);

    priv_nh_.param<int>(node_name+"/interestDistancePoly", interestDistancePoly, 15);
    priv_nh_.param<int>(node_name+"/interestDistanceDefault", interestDistanceDefault, 50);
    
    priv_nh_.param<int>(node_name+"/iterationsRansac", iterationsRansac, 30);
    priv_nh_.param<double>(node_name+"/proportionThreshould", proportionThreshould, 0.6);
    ...
    priv_nh_.param<int>(node_name+"/laneMarkingSquaredThreshold", laneMarkingSquaredThreshold, 1400);

    priv_nh_.param<int>(node_name+"/angleAdjacentLeg", angleAdjacentLeg, 200);
    
    priv_nh_.param<int>(node_name+"/scanlinesVerticalDistance", scanlinesVerticalDistance,6);
    priv_nh_.param<int>(node_name+"/scanlinesMaxCount", scanlinesMaxCount, 160);

    priv_nh_.param<int>(node_name+"/detectLaneStartX", detectLaneStartX, 2);

    priv_nh_.param<int>(node_name+"/maxAngleDiff", maxAngleDiff, 5);

    priv_nh_.param<int>(node_name+"/polyY1", polyY1, 105);
    ...
    maxYRoi_half = maxYRoi/2;
    maxXRoi_half = maxXRoi/2;
    ...
    if (!rgb_camera_info)
    {
        ...
        rgb_camera_info->width = maxXRoi;
        rgb_camera_info->height = proj_image_h;
    }
...
{
    // ROS_INFO("defaultXLeft:%f ",defaultXLeft);
    ...
    //Mat cut_image = image(cv::Rect(0,80,cam_w,379));
    // image = image(cv::Rect(0,0,cam_w,cam_h));
    ...
    // Mat remapped_image = ipMapper.remap(image);
        ...
        cv::waitKey(1);
        // cv::imshow("remapped_image", remapped_image);
        // cv::waitKey(1);
    #endif
    ...
    vector<FuPoint<int>> laneMarkings = cLaneDetectionFu::extractLaneMarkings(edges);
     //---------------------- DEBUG OUTPUT LANE MARKINGS ---------------------------------//
        ...
        {         
            ...
            cv::circle(transformedImagePaintable,markingLoc,transformedImagePaintable.cols/100,cv::Scalar(0,255,0),-1);         
        }
    ...
    //---------------------- DEBUG OUTPUT GROUPED LANE MARKINGS ---------------------------------//
        ...
        {         
            ...
            cv::Point markingLoc = cv::Point(marking.getX(), marking.getY());
            cv::circle(transformedImagePaintable,markingLoc,transformedImagePaintable.cols/100,cv::Scalar(0,0,255),-1);         
        }
        ...
        cv::Point2d p2(maxXRoi_half+(roi_bottom_w/2),maxYRoi-1);
        cv::Point2d p3(maxXRoi_half+(roi_top_w/2),minYPolyRoi);
        cv::Point2d p4(maxXRoi_half-(roi_top_w/2),minYPolyRoi);
        cv::line(transformedImagePaintable,p1,p2,cv::Scalar(0,200,0));
        cv::line(transformedImagePaintable,p2,p3,cv::Scalar(0,200,0));
        cv::line(transformedImagePaintable,p3,p4,cv::Scalar(0,200,0));
        cv::line(transformedImagePaintable,p4,p1,cv::Scalar(0,200,0));
...
vector<vector<LineSegment<int>> > cLaneDetectionFu::getScanlines() {
    ...
    checkContour.push_back(cv::Point(maxXRoi_half-(roi_bottom_w/2),maxYRoi-1));
    checkContour.push_back(cv::Point(maxXRoi_half+(roi_bottom_w/2),maxYRoi-1));
    checkContour.push_back(cv::Point(maxXRoi_half+(roi_top_w/2),minYPolyRoi));
    checkContour.push_back(cv::Point(maxXRoi_half-(roi_top_w/2),minYPolyRoi));
    
    int scanlineStart = 0;
    int scanlineEnd = maxXRoi;
...
vector<vector<EdgePoint>> cLaneDetectionFu::scanImage(cv::Mat image) {
    ...
    // scanline length can maximal be image width
    int scanlineMaxLength = image.cols; 
    // int scanlineMaxLength = maxYRoi; 

    // ROS_INFO("scanlineMaxLength %d",scanlineMaxLength);
    ...
    for (const auto& line : edges) {
        ...
        for (
            ...
            if (edgePosition->isPositive() and not nextEdgePosition->isPositive()) {
                ...
                double d_square=(candidateStartEdge - candidateEndEdge).squaredMagnitude();
                if ((d_square< laneMarkingSquaredThreshold)&&(d_square > 10)) {
...
void cLaneDetectionFu::buildLaneMarkingsLists(
    ...
    for (FuPoint<int> laneMarking : laneMarkings) {
        ...
        {
            if (polyDetectedLeft) {
                if (isInPolyRoiLeft(polyLeft, laneMarking)) {
...
bool cLaneDetectionFu::isInPolyRoi(NewtonPolynomial &poly, FuPoint<int> &p) {
    ...
    else if (horizDistanceToPolynomial(poly, p) <= interestDistancePoly+((maxYRoi-p.getY())/(slop/10))) {
        return true;
    }
    else {
        return false;
...
bool cLaneDetectionFu::isInPolyRoiLeft(NewtonPolynomial &poly, FuPoint<int> &p) {
    if (p.getY() < minYPolyRoi || p.getY() > maxYRoi) {
        return false;
    }
    else if (horizDistanceToPolynomial(poly, p) <= interestDistancePoly+((maxYRoi-p.getY())/(slop_left/10))) {
        return true;
    }
    else {
        return false;
...
/*void LaneDetector::   `(ePosition position) {
    lanePoly.clear();
...
bool cLaneDetectionFu::ransacInternal(ePosition position,
    ...
    for (std::vector<FuPoint<int>>::size_type i = 0; i != sortedMarkings.size();
            i++) {
        if (i < double(sortedMarkings.size()) / max_size) {
            markings1.push_back(sortedMarkings[i]);
        }
        else if (i >= (double(sortedMarkings.size()) / max_size) * 3
                && i < (double(sortedMarkings.size()) / max_size) * 4) {
            markings2.push_back(sortedMarkings[i]);
        }
        else if (i >= (double(sortedMarkings.size()) / max_size) * 6) {
    ...
    while (iterations < iterationsRansac) {
        ...
        // poly.addData(p4.getX(), p4.getY());
        // poly.addData(p5.getX(), p5.getY());
        ...
        if (count1 == 0 || count2 == 0 || count3 == 0) {
            poly.clear();
            // ROS_INFO("Poly had no supporters in one of the regions");
            continue;
        }
        // calculate the proportion of supporters of all lane markings
        double proportion = (double(count1) / markings1.size()
                + double(count3) / markings3.size()
                + 3.0 * (double(count2) / markings2.size())) / 5.0;

        if (proportion < proportionThreshould) {
            poly.clear();
            // ROS_INFO("Poly proportion was smaller than threshold %f",proportion);
            continue;
        }
...
bool cLaneDetectionFu::polyValid(ePosition position, NewtonPolynomial poly,
    ...
    if (prevPoly.getDegree() != -1) {
        ...
        if (horizDistance(p4, p5) > 50) {//0.1 * meters) {
            // ROS_INFO("Poly was to far away from previous poly at y = %d",polyY1);
            return false;
        }
        ...
        if (horizDistance(p6, p7) > 80) {//0.1 * meters) {
            // ROS_INFO("Poly was to far away from previous poly at y = %d",polyY2);
            return false;
        }
    }
    // else
    ...
    //     if (horizDistanceToDefaultLine(position, p1) > 50) {
    //         ROS_INFO("Poly was to far away from default line at y = %d",polyY1);
    //         return false;
    //     }

    //     FuPoint<int> p2 = FuPoint<int>(poly.at(polyY2), polyY2);    //y = 60

    //     if (horizDistanceToDefaultLine(position, p2) > 50) {
    //         ROS_INFO("Poly was to far away from default line at %d",polyY1);
    //         return false;
    //     }

    //     FuPoint<int> p3 = FuPoint<int>(poly.at(polyY3), polyY3);    //y = 40

    //     if (horizDistanceToDefaultLine(position, p3) > 50) {
    //         ROS_INFO("Poly was to far away from default line  %d",polyY1);
    //         return false;
    //     }
    // }
